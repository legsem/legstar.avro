...
public class Cob2AvroUnmarshalVisitor extends CobolElementVisitor {
    ...
    public void visit(ICobolComplexBinding ce) throws HostException {
        ...
        for (ICobolBinding child : ce.getChildrenList()) {
            ...
            if (child instanceof ICobolComplexBinding) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("Restored avro record to "
                            + avroRecord.getSchema());
                    log.debug(
                            "End visiting field {} with avro schema {} value={}",
                            avroFieldName, avroSchema,
     ...
     */
    private void visitAlternative(List < ICobolBinding > alternativesList,
            ICobolBinding alt) throws HostException {
     ...
     * unmarshaled to the alternative (rather tha the parent avro record).
     * 
     * @param altIndex index of the alternative to be visited in the
     *            alternatives list
     * @param alt the alternative to be visited
     * @throws HostException
     */
    private void visitAlternative(int altIndex, ICobolBinding alt)
            throws HostException {
        GenericRecord avroRecord = currentAvroRecord;
        Schema avroChoiceAlternativeSchema = currentAvroChoiceAlternativeSchema;
        Schema avroChoiceSchema = getSchema(avroRecord,
                currentAvroChoiceFieldName);
        try {
            String avroTypeName = getAvroTypeName(alt);
            currentAvroChoiceAlternativeSchema = avroChoiceSchema.getTypes()
                    .get(altIndex);
            if (currentAvroChoiceAlternativeSchema.getType() == Schema.Type.RECORD) {
                currentAvroRecord = new GenericData.Record(
                        currentAvroChoiceAlternativeSchema);
    ...
    public void visit(ICobolArrayComplexBinding ce) throws HostException {
        ...
        if (log.isDebugEnabled()) {
            log.debug(
                    ...
                    ce.getJaxbName(), ce.getCurrentOccurs(),
                    avroArray.getSchema());
        }
    ...
    private void store(ICobolOctetStreamBinding ce) throws HostException {
        String fieldName = getAvroFieldName(ce);
        Schema schema = getSchema(fieldName);
        GenericData.Fixed genericFixed = new GenericData.Fixed(schema,
     ...
     * <p/>
     * If the field is not in the parent record, assume it is the current choice
     * alternative schema.
     * 
     ...
     */
    private Schema getSchema(String fieldName) throws HostException {
     ...
     * <p/>
     * If the field name is not in the avro record schema it might be because we
     * are populating a choice rather than the record itself.
     * 
    ...
    private <T extends ICobolBinding> String getAvroFieldName(T ce) {
        return StringUtils.uncapitalize(ce.getJaxbName());
    }

    private <T extends ICobolBinding> String getAvroTypeName(T ce) {
        return StringUtils.capitalize(ce.getJaxbName());
    }
