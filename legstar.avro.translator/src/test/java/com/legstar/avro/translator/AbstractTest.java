...
public class AbstractTest {
    ...
    public static final File TEST_JAVA_FOLDER = new File("target/test/gen");
	 ...
	 */
	public void check(final String resultText, String refFileName) throws Exception {
		File refFile = new File(SRC_REF_DIR, name.getMethodName() + "/" + refFileName);
		if (isCreateReferences()) {
			FileUtils.writeStringToFile(refFile, resultText, Charsets.UTF_8);
		} else {
			String referenceText = FileUtils.readFileToString(refFile, Charsets.UTF_8);
